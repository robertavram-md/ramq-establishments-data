...
def parse_establishments_with_address_and_categories(text, region_name):
    ...
    while i < len(lines):
        ...
        if code_match:
            ...
            # Extract categories from the end of the line
            categories = []
            ...
            if i + 1 < len(lines):
                ...
                # If next line doesn't start with a code and looks like an address
                if not re.search(r'^\d{4}[0-9X]\b', next_line) and next_line:
                    # Check if this looks like an address (contains postal code or city)
                    if re.search(r'[A-Z][0-9][A-Z]\s*[0-9][A-Z][0-9]|QC,|QUEBEC|MONTREAL', next_line.upper()):
                        address = next_line
